...
class TimesFM_2p5_200M_torch(timesfm_2p5_base.TimesFM_2p5, ModelHubMixin):
  ...
  def _from_pretrained(
    ...
    **model_kwargs,
    ...
    # Download the config file for hf tracking (only if not in local_files_only mode).
    if not local_files_only:
      try:
        ...
        print("Downloaded config.json from Hugging Face Hub.")
      except Exception as e:
        logging.warning("Failed to download config.json: %s", e)
